...
HTML=$(curl -L -sS -c "${COOKIE_FILE}" -w 'LAST_URL:%{url_effective}' ${TEST_SITE}) || (end 2 "Failed to fetch URL: ${TEST_SITE}")
...
-d "j_username=$LOGIN" -d  "j_password=$PASSWORD" --data-urlencode "AuthState=${AUTH_STATE}" ${AUTH_URL}) || (end 2 "Failed to fetch URL: ${AUTH_URL}")
...
HTML=$(curl -L -s -c "${COOKIE_FILE}" -b "${COOKIE_FILE}" -w 'LAST_URL:%{url_effective}' \
  --data-urlencode "SAMLResponse=${SP_RESPONSE}" --data-urlencode "RelayState=${SP_RELAY_STATE}" "${SP_ENDPOINT}") || (end 2 "Failed to fetch URL: ${SP_ENDPOINT}" )
