...
# This script is used make a full roundtrip login test to OIDC SP via CESNET account
...
HTML=$(curl -L -s -c "${COOKIE_FILE}" -w 'LAST_URL:%{url_effective}' --resolve "${DOMAIN_NAME}"':443:'${IP} "${TEST_SITE}") || (end 2 "Failed to fetch URL: ${TEST_SITE}")
...
HTML=$(curl -L -s -c "${COOKIE_FILE}" -b "${COOKIE_FILE}" -w 'LAST_URL:%{url_effective}' -d "j_username=$LOGIN" \
  -d  "j_password=$PASSWORD" -d "_eventId_proceed=" --resolve "${DOMAIN_NAME}"':443:'${IP} -d "csrf_token=$CSRF_TOKEN" "${AUTH_URL}") || (end 2 "Failed to fetch URL: ${AUTH_URL}")
