...
CACHE_FILE=${DIR}/proxy_idp_auth_test_oidc.cache
...
if [[ -f "$CACHE_FILE" ]]; then
  ...
  REGEX_CHECK=$(grep -c '\d proxy_idp_auth_test_oidc.sh.*muni_login_time.*cesnet_login_time.*MUNI STATUS.*CESNET STATUS' ${CACHE_FILE})
...
BASENAME=$(basename "$0")
WARNING_TIME=${OIDC_WARNING_TIME}
