...
if [ ${AAI_RC} -eq 0 ]; then
    ...
    RESULT="SAML Auth test active - ${STATUS_TXT} [AAI STATUS - ${AAI_RESULT}(${AAI_LOGIN_TIME}s); MUNI STATUS - Not tried; CESNET STATUS - Not tried] | aai_login_time=${AAI_LOGIN_TIME} muni_login_time=0 cesnet_login_time=0"
    echo ${RESULT}
    exit ${STATUS}
fi
...
if [ ${MUNI_RC} -eq 0 ]; then
    ...
    RESULT="SAML Auth test active - ${STATUS_TXT} [AAI STATUS - ${AAI_RESULT}(${AAI_LOGIN_TIME}s); MUNI STATUS - ${MUNI_RESULT}(${MUNI_LOGIN_TIME}s); CESNET STATUS - Not tried] | aai_login_time=${AAI_LOGIN_TIME} muni_login_time=${MUNI_LOGIN_TIME} cesnet_login_time=0"
    echo ${RESULT}
    exit ${STATUS}
fi
...
RESULT="SAML Auth test active - ${STATUS_TXT} [AAI STATUS - ${AAI_RESULT}(${AAI_LOGIN_TIME}s); MUNI STATUS - ${MUNI_RESULT}(${MUNI_LOGIN_TIME}s); CESNET STATUS - ${CESNET_RESULT}(${CESNET_LOGIN_TIME}] | aai_login_time=${AAI_LOGIN_TIME} muni_login_time=${MUNI_LOGIN_TIME} cesnet_login_time=${CESNET_LOGIN_TIME}"
echo ${RESULT}
exit ${STATUS}
