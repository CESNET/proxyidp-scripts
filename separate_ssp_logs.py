...
for line in file:
    ...
    if session_id is None or session_id.group(1) not in session_ids:
        thread_id = re.search(regex_thread_id, line)
        if thread_id is None or thread_id.group(1) not in thread_ids or last_session_id not in session_ids:
...
# Zip old log file
with open(full_log_absolute_name, 'rb') as f_in, gzip.open(full_log_absolute_name + '.gz' , 'wb') as f_out:
